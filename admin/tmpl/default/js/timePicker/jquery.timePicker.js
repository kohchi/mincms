...
/*
 * Modified by Kazuo Kohchi <kohchi@miyagino.net>
 * Add a second option that add "00" second. Default is true.
 *
 * @example  $("#mytime").timePicker({second: true});
 */
...
(function($){
	...
	$._timePicker = function(elm, settings) {
		...
		$(elm).unbind().focus(showPicker).click(showPicker).blur(function() { // Hide timepicker on blur
			if (!tpOver && $tpDiv[0].parentNode) { // Don't remove when timePicker is clicked or when already removed
				$tpDiv.hide();
			}
		}).keypress(function(e) { // Key support
			switch (e.keyCode) {
			case 38: // Up arrow.
	...
	function formatTime(time, settings) {
		...
		return formatNumber(hours) + settings.separator + formatNumber(minutes) + (settings.show24Hours ? '' : ((h < 12) ? ' AM' : ' PM')) +
			(settings.second ? (settings.separator + '00') : '');
	}
